#!/usr/bin/env python3
"""Unbudgeted whole-graph 5-color SAT solve and exact pair separation.

Ordinary full-graph mode uses a sound triangle color-symmetry break.  Pair mode
uses the stronger WLOG fact that any proper coloring with u!=v can be globally
permuted so color(u)=0 and color(v)=1.  Thus SAT under those two assumptions is
exactly equivalent to existence of a separating 5-coloring; UNSAT makes the pair
a forced-equal B candidate requiring independent confirmation.

Iterative mode repeatedly targets the largest remaining same-signature block using
the same WLOG pair assumptions. Every retained SAT model is validated on every
exact geometric edge before it refines the witness signatures.
"""
...
def run_iterative(a,raw,n,edges,clauses,seed):
    ...
    with cls(bootstrap_with=clauses) as s:
        for i in range(a.iterate_pairs):
            ...
            # Sound WLOG: any separating coloring can be globally renamed so u=0,v=1.
            pair_assumptions=[u*5+1,v*5+2]
            ans=s.solve(assumptions=pair_assumptions)
            if ans is False:
            ...
            colors=extract(s.get_model(),n); assert valid_coloring(colors,n,edges); assert colors[u]==0 and colors[v]==1
...
def main():
    ...
    if a.iterate_pairs:
        ...
        run_iterative(a,raw,n,edges,clauses,seed); return
    pair=None
    if a.neq_pair is not None:
        ...
        symmetry={'pair_symmetry_wlog':'color(u)=0,color(v)=1 by global color permutation','pair_symmetry_sound':True}
    else:
        tri=find_triangle(n,edges); assert tri is not None
        assumptions=[tri[0]*5+1,tri[1]*5+2,tri[2]*5+3]
        symmetry={'triangle':list(tri),'symmetry_assumptions_sound':True}
    cls=SOLVERS[a.solver]
    with cls(bootstrap_with=clauses) as s:
    ...
    if ans:
        ...
        if pair is not None: assert colors[pair[0]]==0 and colors[pair[1]]==1
        out={'status':'SAT','classification':'NOT_A' if pair is None else 'NOT_FORCED_EQUAL','solver':a.solver,'vertices':n,'edges':len(edges),'graph_sha256':hashlib.sha256(raw).hexdigest(),'all_edges_validated':True,'colors':colors,**symmetry}
        if pair is not None: out.update({'pair':pair,'pair_colors':[0,1],'pair_inequality_validated':True})
        (a.out_dir/'COLORING.txt').write_text(''.join(map(str,colors))+'\n')
    else:
        out={'status':'UNSAT','classification':'A_CANDIDATE_REQUIRES_CERTIFICATE' if pair is None else 'B_CANDIDATE_REQUIRES_INDEPENDENT_CONFIRMATION','solver':a.solver,'vertices':n,'edges':len(edges),'graph_sha256':hashlib.sha256(raw).hexdigest(),**symmetry}
