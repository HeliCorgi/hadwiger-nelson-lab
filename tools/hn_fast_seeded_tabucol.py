...
on every iteration. Any returned coloring is independently validated on every
saved exact edge. Failure or timeout is non-evidence.
"""
...
def search(seed,edges,k,rng_seed,restarts,iterations,seconds,perturb):
    ...
    for restart in range(restarts):
        ...
        for it in range(iterations):
            ...
            # Neighbor conflict status can change when their count of old/c changes.
            for w in adj[v]:
...
def main():
    ...
    color,records=search(seed,edges,a.colors,a.seed,a.restarts,a.iterations,a.seconds,a.perturb)
    if color is None:
        ...
        out={'status':'VALIDATED_PROPER_5_COLORING','classification':'NOT_A','vertices':n,'edges':len(edges),'seed_conflicts':start,'all_edges_validated':True,'colors':color,'records':records}
